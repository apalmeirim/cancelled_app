...
export default function Modal({ title, description, isOpen, onClose, children, className }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 py-10 sm:px-6">
      ...
      <div
        className={cn(
          "relative z-[70] w-full max-w-3xl rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950/95 via-slate-900/90 to-slate-950/85 shadow-[0_22px_60px_rgba(8,15,40,0.65)]",
          "p-6 sm:p-8",
          className
        )}
      >
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            {title ? <h2 className="text-2xl font-semibold text-emerald-200">{title}</h2> : null}
